...
- This system is a web-based exam creation and correction tool.
- Client: React + TypeScript (Vite).
- Server: Node + Express + TypeScript.
- Database: SQLite via better-sqlite3 and Drizzle ORM. The DB file lives at `data/app.db`.
- Tests: Cucumber/Gherkin scenarios in `tests/features/`, step definitions in
  `tests/steps/`. Run with `npx cucumber-js`.
- Every new feature must have Gherkin scenarios written before implementation.
- All database writes touching more than one table must use better-sqlite3 transactions.
- No route handler may contain more than 10 lines of business logic.
- All TypeScript must compile with strict mode and zero `any` types.

# Operating principles
- **TDD first — no exceptions.** For every feature, the order is always:
  1. Write (or update) the Gherkin `.feature` file.
  2. Implement the step definitions — run them, watch them fail (red).
  3. Implement the minimum server/client code to make them pass (green).
  4. Refactor while keeping tests green.
  Never write production code before a failing test exists for it.
 
- **Ambitious & persistent.** Pursue each task until its acceptance criteria are fully
  satisfied. When facing ambiguity, make the most reasonable assumption, proceed, and
  document the assumption inline as a `// ASSUMPTION:` comment.
 
- **High signal, low noise.** Emit short, outcome-focused updates. Prefer diffs and test
  output over prose narration. Keep chat messages brief.
 
- **Safe autonomy.** For wide/risky edits (mass rename, schema change, dependency
  upgrade), prepare a brief **Destructive Action Plan (DAP)** — scope, rollback, risk,
  validation — and pause for explicit approval before proceeding.
 
- **Conflict rule.** When guidance conflicts: **safety > correctness > TDD discipline > speed**.

# TDD workflow (mandatory)
 
```
┌─────────────────────────────────────────────────────────┐
│  For each acceptance criterion / user story:            │
│                                                         │
│  1. FEATURE FILE  →  write / extend *.feature           │
│  2. STEP DEFS     →  implement steps, run cucumber      │
│                       → confirm RED (failing)           │
│  3. IMPLEMENT     →  server route / service / client UI │
│  4. RUN TESTS     →  npx cucumber-js                    │
│                       → confirm GREEN (passing)         │
│  5. REFACTOR      →  improve code, keep GREEN           │
│  6. COMMIT        →  one commit per green step          │
└─────────────────────────────────────────────────────────┘
```
 
Never skip step 2 (the red phase). If a test passes immediately without any production
code written, the test is not testing anything — investigate and fix it.
